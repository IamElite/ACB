...
async def generate_image_handler(client: Client, message: Message):
    ...
    try:
        ...
        if not img_urls:
            await message.reply("ᴋᴏɪ ɪᴍᴀɢᴇ ɴᴀʜɪ ᴍɪʟɪ, ᴅᴜʙᴀʀᴀ ᴛʀʏ ᴋᴀʀᴏ!", reply_to_message_id=reply_to.message_id)
            await waiting_message.delete()
            return

        saved_files = await image_generator.download_images(img_urls)
        if not saved_files:
            await message.reply("ɪᴍᴀɢᴇ ꜱᴀᴠᴇ ᴋᴀʀɴᴇ ᴍᴇ ᴅɪᴋᴋᴀᴛ ᴀᴀʏɪ!", reply_to_message_id=reply_to.message_id)
            await waiting_message.delete()
            return

        # Sabhi images ek saath bhej do
        media_group = [
            client.send_photo(chat_id=message.chat.id, photo=file, reply_to_message_id=reply_to.message_id)
        ...
        await message.reply(f"ᴇʀʀᴏʀ ᴀᴀʏᴀ: {str(e)}", reply_to_message_id=reply_to.message_id)
