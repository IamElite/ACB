...
import datetime, html, textwrap, bs4, jikanpy, requests
...
def upcoming(client, message):
    ...
    message.reply_text(upcoming_message)


def site_search(client, message, site: str):
    search_query = extract_arg(message)
    more_results = True
...
def kaizoku(client, message):
    site_search(client, message, "kaizoku")


def kayo(client, message):
    site_search(client, message, "kayo")
...
# Create handlers for pyrogram
def register_handlers():
    app.add_handler(MessageHandler(anime, filters.command("anime")))
    app.add_handler(MessageHandler(airing, filters.command("airing")))
    app.add_handler(MessageHandler(character, filters.command("character")))
    app.add_handler(MessageHandler(manga, filters.command("manga")))
    app.add_handler(MessageHandler(user, filters.command("user")))
    app.add_handler(MessageHandler(upcoming, filters.command("upcoming")))
    app.add_handler(MessageHandler(kaizoku, filters.command("kaizoku")))
    app.add_handler(MessageHandler(kayo, filters.command("kayo")))
